...
public class CandidateServiceImp extends GenericServiceImpl<Candidate, CandidateDTO, Long> implements CandidateService {
	...
	private double calculateScore(List<QuizQuestionDTO> quizQuestions) {
		...
		for (QuizQuestionDTO quizQuestionDTO : quizQuestions) {
			quizQuestionTotalScore += quizQuestionDTO.getQuestion().getScore();
			
			List<ProposedResponseDTO> proposedResponses = quizQuestionDTO.getQuestion().getProposedResponses();
			...
			// If RADIO
			if(quizQuestionDTO.getQuestion().getType().equals(Type.RADIO)) {
						...
						if(proposedResponse.getContent().equals(answer.getContent())  && proposedResponse.isState()) {
							...
							for (int i = 0; i < answers.size(); i++) {
								if(answers.get(i).getId() == answer.getId()) {
									answers.get(i).setScore(quizQuestionDTO.getQuestion().getScore());
								}
							}
							
							// Calculate the score ...
							totalAnswersScore += quizQuestionDTO.getQuestion().getScore();
						}
			...
			Long currentquizQuestionId = quizQuestionDTO.getQuestion().getId();
			
			if(quizQuestionDTO.getQuestion().getType().equals(Type.CHECKBOX)) {
				
				int trueAnswersSize = answers.size();
				long trueProposedResponsesSize = proposedResponses.stream().filter(proposedResponse -> proposedResponse.isState()).count();
				...
				if(allAnswersIsValid && currentquizQuestionId == quizQuestionDTO.getQuestion().getId()) {
					// Set score to this question
					for (int i = 0; i < answers.size(); i++) {
						for (int j = 0; j < quizQuestionDTO.getAnswers().size(); j++) {
							double questionScore = quizQuestionDTO.getQuestion().getScore();
			...
			if(quizQuestionDTO.getQuestion().getType().equals(Type.TEXTFIELD) || quizQuestionDTO.getQuestion().getType().equals(Type.TEXTAREA)) {
				// Calculate the score ...
